...
#if defined(NDEBUG)
# error "SPEC cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xde197396740f2624322c1735da3563973f8e4c566efdc05ef0ace7e232bc040e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SPEC: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SPEC Signed Message:\n";
...
{
    // SPEC: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SPEC: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SPEC: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xde197396740f2624322c1735da3563973f8e4c566efdc05ef0ace7e232bc040e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1435009803, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "22/Jun/2015 WSJ 14 June 2015 The Dollar Loses Allure";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1435009803;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1478731;
        ...
        assert(block.hashMerkleRoot == uint256("0xcafb86921178f01871f83aa8dd2d32d1c6909368d5a41a965f51cb6f5b0e2fd3"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // SPEC: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SPECMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SPECMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SPECMiner(CWallet *pwallet)
{
    printf("SPECMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spec-miner");
    ...
    try { loop {
        ...
        printf("Running SPECMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SPECMiner, pwallet));
}
