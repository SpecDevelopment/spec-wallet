0.8.7.5 changes
=============
- openssl-1.0.1k or older versions patched for CVE-2014-8275 broke compatibility with Bitcoin and SPEC.
  This update patches SPEC to maintain compatibility with CVE-2014-8275 patched openssl.
- If you are running v0.8.7.4 as distributed by spec.org you do not need to upgrade.
  The binaries distributed on spec.org contain their own copy of openssl so they are unaffected by this issue.
...
- Minor fixes:
  - s/Bitcoin/SPEC/ in the Coin Control example
...
- Internal miner added back to SPEC.  setgenerate now works, although it is generally a bad idea as it is significantly slower than external CPU miners.
...
Fix out-of-bounds check (SPEC currently does not use this codepath, but we apply this
patch just to match Bitcoin 0.8.5.)

0.8.4.1 changes
===============

CVE-2013-5700 Bloom: filter crash issue - SPEC 0.8.3.7 disabled bloom by default so was 
...
nodes running with the -debug flag to crash, although it was lot less likely on SPEC 
